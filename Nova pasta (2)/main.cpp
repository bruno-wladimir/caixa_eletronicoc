...
cout<<("\t########################################################\n");
       ...
       cout<<("\t########################################################\n\n\n");
				...
				fflush(stdin);
				for(i=0;i<6; i++){
				Login.senha_cadastro[i] = getch();
				putchar('*');
				Login.senha_cadastro[i] = toupper(Login.senha_cadastro[i]);}
				
				cout<<("\n");
				 
				Login.senha_cadastro[i]='\0';
...
switch (opc_tela_login){
	...
	fflush(stdin);
for (i=0;i<6;i++){
	Login.senha_acesso[i]= getch();
	putchar('*');
	Login.senha_acesso[i] = toupper(Login.senha_acesso[i]);
	
	Login.senha_acesso[i]= '\0';
}

	cout<<("\n");
	
	if (Login.cpf_acesso== Login.cpf && Login.senha_acesso[6]== Login.senha_cadastro[6])
